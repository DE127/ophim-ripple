...
class RippleController
{
    ...
    public function getMovieOfCategory(Request $request)
    {
        ...
        $movies = $category->movies()->orderBy('publish_year', 'desc')->paginate(get_theme_option('per_page_limit'));
    ...
    public function getMovieOfRegion(Request $request)
    {
        ...
        $movies = $region->movies()->orderBy('publish_year', 'desc')->paginate(get_theme_option('per_page_limit'));
    ...
    public function getMovieOfActor(Request $request)
    {
        ...
        $movies = $actor->movies()->orderBy('publish_year', 'desc')->paginate(get_theme_option('per_page_limit'));
    ...
    public function getMovieOfDirector(Request $request)
    {
        ...
        $movies = $director->movies()->orderBy('publish_year', 'desc')->paginate(get_theme_option('per_page_limit'));
    ...
    public function getMovieOfTag(Request $request)
    {
        ...
        $movies = $tag->movies()->orderBy('publish_year', 'desc')->paginate(get_theme_option('per_page_limit'));
        return view('themes::ripple.catalog', [
            'data' => $movies,
            'tag' => $tag,
            'title' => $tag->getTitle(),
    ...
    public function getMovieOfType(Request $request)
    {
        ...
        if(is_null($movies)) {
            ...
            [$relation_config, $field, $val, $sortKey, $alg] = array_merge($value, ['', 'is_copyright', 0, 'publish_year', 'desc']);
